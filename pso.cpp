...
pso_settings_t *pso_settings_new(std::vector<double> range_lo, std::vector<double> range_hi, double goal, int swarm_size, int max_steps, double delta, double low_delta_count_limit)
{
	if (range_lo.size() != range_hi.size())
	{
		printf("Different number of low range and high range!");
		exit(0);
	}

	pso_settings_t *settings = pso_settings_new(range_lo.size(),0,0,goal, swarm_size,max_steps, delta, low_delta_count_limit);
...
pso_settings_t *pso_settings_new(int dim, double range_lo, double goal, double range_hi, int swarm_size, int max_steps, double delta, double low_delta_count_limit) {
	...
	settings->low_delta_count_limit = low_delta_count_limit;

	if (swarm_size < 1)
		settings->size = pso_calc_swarm_size(settings->dim);
	else
		settings->size = swarm_size;
    ...
    settings->nhood_strategy = PSO_NHOOD_GLOBAL;
	       ...
	       pso_result_t *solution, pso_settings_t *settings)
{
	...
	solution->delta_error = DBL_MAX;
	solution->error_before = DBL_MAX;
	solution->low_delta_count = 0;
	...
	solution->error_before = solution->error;
    ...
    for (step=0; step<settings->steps; step++) {
		...
		solution->delta_error <= settings->delta ? solution->low_delta_count++ : solution->low_delta_count = 0;
		
		if(solution->low_delta_count>=settings->low_delta_count_limit)
		{
			// SOLVED!!
			if (settings->print_every)
				printf("Error not improving @ step %d (error=%.3e, delta error=%.3e :-)\n", step, solution->error, solution->delta_error);
			break;
		}
		...
		solution->delta_error = solution->error_before - solution->error;
		solution->error_before = solution->error;


		if (settings->print_every && (step % settings->print_every == 0))
		{
			printf("Step %d (w=%.2f) :: min err=%.5e, low_delta_count %f\n", step, w, solution->error, solution->low_delta_count);
			/*for (int i = 0; i < 8; i++)
				printf("%.3f ", solution->gbest[i]);
			printf("\n");*/
		}
